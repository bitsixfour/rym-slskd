import { getArtist, getTitle } from "./background.js";
const apiKey = "dccee590-328f-4a16-a350-a728584eafd8";
const url = "http://192.168.1.12:5030";

async function slskdRequest() {
  let request = fetch(url + "/api/v0/searches", {
    method: "POST",
    headers: {
      "X-API-Key": apiKey,
    },
    body: JSON.stringify({
      searchText: getArtist() + " " + getTitle(),
    }),
  });
...
async function slskdStatus() {
  let status = await fetch(url + "/api/v0/searches/" + getId(), {
    method: "GET",
    headers: {
      "X-API-Key": apiKey,
    },
...
function returnArray() {
  return JSON.parse(fetch(url + "/api/v0/searches/" + getId() + "/responses"));
}
...
async function slskdDownload() {
  ...
  for (let i = target; i < arr.length; i++) {
    ...
    if (
      ...
      (getArtist() + getTitle()).toLowerCase().includes("live") != true
    ) {
      continue;
    }
  ...
  let downloadStatus = await fetch(
    url + "/api/v0/transfers/downloads/" + user,
    {
      method: "POST",
      headers: {
        "X-API-Key": apiKey,
